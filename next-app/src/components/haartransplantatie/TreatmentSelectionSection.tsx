...
export const TreatmentSelectionSection = () => {
  ...
  // Local state to track active tab independently of global session
  const [localPackage, setLocalPackage] = useState<Package>(profile.selectedPackage);
  ...
  const getSpacing = () => {
    if (typeof window === 'undefined') return { container: '', country: '', package: '', content: '' };
      ...
      <div className={`flex-1 flex flex-col justify-start px-4 relative z-10`}>
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (!disabled) {
                        ...
                        setLocalPackage(pkg.id as Package);

                        // Sync to global state (optional, for safety, but UI relies on local)
                        updateProfile('selectedPackage', pkg.id);
                      }
                    }}
                    className={`px-3 py-2.5 rounded-full font-lato text-[13px] font-medium transition-all duration-300 ease-out relative overflow-hidden ${disabled
                        ? 'opacity-40 cursor-not-allowed text-white/50'
                        : localPackage === pkg.id
                          ? 'text-white premium-glow'
                          : 'text-white/80 hover:text-white hover:bg-white/5'
                      }`}
                    style={!disabled && localPackage === pkg.id ? {
        ...
        >
          <div className="max-w-56 mx-auto">
            <NewPackageContent packageOverride={localPackage} />
          </div>
        </div>
      </div>
      {/* Right Side Icons */}
      <div className="fixed right-3 sm:right-4 md:right-5 lg:right-6 bottom-20 sm:bottom-24 md:bottom-28 lg:bottom-32 xl:bottom-36 space-y-3 z-50">
        <button
          onClick={() => {
            ...
            router.push(language === 'nl' ? '/nl/info/traject' : '/en/info/trajectory');
          }}
          className={`w-12 h-12 sm:w-14 sm:h-14 bg-black/18 backdrop-blur-lg rounded-full flex items-center justify-center border border-white/50 hover:border-white/70 hover:bg-black/22 transition-all duration-700 ease-in-out cursor-pointer ${buttonsLoaded[0] ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'
            }`}
        >
          <svg
            width="24"
        ...
        <button
          onClick={() => {
            // Store current path before navigating to popup
            sessionStorage.setItem('previousPath', window.location.pathname);
            router.push(language === 'nl' ? '/nl/support' : '/en/support');
          }}
          className={`w-12 h-12 sm:w-14 sm:h-14 bg-black/18 backdrop-blur-lg rounded-full flex items-center justify-center border border-white/50 hover:border-white/70 hover:bg-black/22 transition-all duration-700 ease-in-out cursor-pointer ${buttonsLoaded[1] ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'
            }`}
        >
          <svg
            width="24"
